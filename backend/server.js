...
function validarProductor(productor, gruposGanado) {
    ...
    if (productor.fecha_alta) {
        ...
        if (fechaAlta > hoy) errores.push('La fecha de alta no puede ser posterior al día de hoy.');
    }
    if (productor.telefono) {
        const telefonoLimpio = productor.telefono.replace(/\D/g, '');
        if (telefonoLimpio.length !== 10) errores.push('El teléfono debe contener exactamente 10 dígitos numéricos.');
    }
    if (!gruposGanado || gruposGanado.length === 0) {
        ...
        gruposGanado.forEach((grupo, idx) => {
            ...
            if (crias > vientres) errores.push(`En el grupo ${idx+1}, las crías (${crias}) no pueden ser mayores que los vientres (${vientres}).`);
        });
        if (totalCabezas === 0) errores.push('El total de cabezas no puede ser cero. Registre al menos un animal.');
    }
    return errores;
}
...
async function calcularRiesgoMunicipios() {
    ...
    const ganado = await pool.query(`
        ...
        FROM productores p LEFT JOIN grupos_ganado gg ON p.id = gg.productor_id GROUP BY p.municipio
    `);
    const casos = await pool.query(`
        SELECT municipio, COUNT(*) as num_casos, SUM(num_animales_afectados) as animales_afectados
        FROM casos_sanitarios WHERE enfermedad = 'barrenador' AND fecha > CURRENT_DATE - INTERVAL '90 days' GROUP BY municipio
    `);
    const riesgoMap = {};
    for (const prod of productores.rows) {
        ...
        riesgoMap[municipio] = {
            riesgo: parseFloat(riesgoTotal.toFixed(2)), nivel,
            densidad: parseFloat(densidad.toFixed(1)), numCasos, animalesAfectados, temperatura, humedad
        };
    }
    for (const [municipio, data] of Object.entries(riesgoMap)) {
        await pool.query(`
            ...
            ON CONFLICT (municipio) DO UPDATE SET
                fecha_calculo = NOW(), riesgo_total = EXCLUDED.riesgo_total,
                nivel = EXCLUDED.nivel, factores = EXCLUDED.factores
...
app.get('/api/buscar', async (req, res) => {
    const { q } = req.query;
    try {
        const result = await pool.query('SELECT * FROM productores WHERE folio = $1 OR telefono = $1', [q]);
        res.json(result.rows[0] || null);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/productores/:id', async (req, res) => {
    ...
    try {
        ...
        if (productorRes.rows.length === 0) return res.status(404).json({ error: 'Productor no encontrado' });
...
app.post('/api/productores', async (req, res) => {
    ...
    if (productor.upp && productor.upp.trim() !== '') {
        ...
        if (uppExistente.rows.length > 0) return res.status(400).json({ error: 'Ya existe un productor con esa clave UPP.' });
    }
    if (productor.folio && productor.folio.trim() !== '') {
        ...
        if (folioExistente.rows.length > 0) return res.status(400).json({ error: 'Ya existe un productor con ese folio.' });
    }
    ...
    try {
        ...
        const insertRes = await client.query(
            ...
            (nombre, rancho, municipio, localidad, tenencia, telefono, fecha_alta, patente, uma, colonia, calle, upp, upps_adicionales, figura_fierro, folio, observaciones)
            ...
            [
                ...
                productor.uma, productor.colonia, productor.calle, productor.upp,
                productor.upps_adicionales || null, productor.figura_fierro || null,
                productor.folio, productor.observaciones
            ]
        );
        ...
        for (const grupo of gruposGanado) {
            await client.query(
                ...
                (productor_id, especie, raza, cruza, vientres, crias, sementales, vaquillas, novillos, engorda, otras_cant, otras_espec, upp_asociada, arete)
                ...
                [
                    ...
                    grupo.upp_asociada || null, grupo.arete || null
                ]
            );
        }
...
app.put('/api/productores/:id', async (req, res) => {
    ...
    if (productor.upp && productor.upp.trim() !== '') {
        ...
        if (uppExistente.rows.length > 0) return res.status(400).json({ error: 'Ya existe otro productor con esa clave UPP.' });
    }
    if (productor.folio && productor.folio.trim() !== '') {
        const folioExistente = await pool.query('SELECT id FROM productores WHERE folio = $1 AND id != $2', [productor.folio, id]);
        if (folioExistente.rows.length > 0) return res.status(400).json({ error: 'Ya existe otro productor con ese folio.' });
    }
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await client.query(
            `UPDATE productores SET
                ...
                fecha_alta=$7, patente=$8, uma=$9, colonia=$10, calle=$11, upp=$12,
                upps_adicionales=$13, figura_fierro=$14, folio=$15, observaciones=$16
            WHERE id=$17`,
            [
                productor.nombre, productor.rancho, productor.municipio, productor.localidad,
                productor.tenencia, productor.telefono, productor.fecha_alta, productor.patente,
                productor.uma, productor.colonia, productor.calle, productor.upp,
                productor.upps_adicionales || null, productor.figura_fierro || null,
                productor.folio, productor.observaciones, id
            ]
        );
...
app.put('/api/ventanillas/:municipio', async (req, res) => {
    ...
    try {
        await pool.query('UPDATE ventanillas SET usuario = $1, password = $2 WHERE municipio = $3', [usuario, password, municipio]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/ventanillas/validar', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT municipio FROM ventanillas WHERE usuario = $1 AND password = $2', [username, password]);
        if (result.rows.length > 0) res.json({ success: true, municipio: result.rows[0].municipio });
...
app.get('/api/estadisticas', async (req, res) => {
    try {
        const productoresPorMunicipio = await pool.query('SELECT municipio, COUNT(*) as total_productores FROM productores GROUP BY municipio');
        const cabezasPorMunicipio = await pool.query(`
            SELECT p.municipio, COALESCE(SUM(gg.vientres + gg.crias + gg.sementales + gg.vaquillas + gg.novillos + gg.engorda + gg.otras_cant), 0) as total_cabezas
            FROM productores p LEFT JOIN grupos_ganado gg ON p.id = gg.productor_id GROUP BY p.municipio
        `);
        const stats = {};
        productoresPorMunicipio.rows.forEach(row => { stats[row.municipio] = { total_productores: parseInt(row.total_productores), total_cabezas: 0 }; });
        cabezasPorMunicipio.rows.forEach(row => {
            if (stats[row.municipio]) stats[row.municipio].total_cabezas = parseInt(row.total_cabezas);
            else stats[row.municipio] = { total_productores: 0, total_cabezas: parseInt(row.total_cabezas) };
        });
        res.json(stats);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/riesgo', async (req, res) => {
    try {
        if (req.query.recalcular === 'true') await calcularRiesgoMunicipios();
...
app.post('/api/casos', async (req, res) => {
    ...
    if (!municipio || !enfermedad) return res.status(400).json({ error: 'Municipio y enfermedad son requeridos' });
    try {
        await pool.query('INSERT INTO casos_sanitarios (municipio, enfermedad, fecha, num_animales_afectados, fuente) VALUES ($1, $2, CURRENT_DATE, $3, $4)', [municipio, enfermedad, num_animales_afectados || null, fuente || 'manual']);
...
app.get('/api/prediccion-ml', async (req, res) => {
    try {
        ...
        `);
        const casos = await pool.query(`
            SELECT municipio, COUNT(*) as num_casos FROM casos_sanitarios
            WHERE enfermedad = 'barrenador' AND fecha > CURRENT_DATE - INTERVAL '90 days' GROUP BY municipio
        `);
        ...
        exec(`python3 ${scriptPath} '${inputJson}'`, (error, stdout, stderr) => {
            if (error) return res.status(500).json({ error: 'Error en el modelo de IA' });
            try {
                ...
                const respuesta = nombres.map((nombre, idx) => {
                    ...
                    return { municipio: nombre, riesgo: parseFloat(prob.toFixed(2)), nivel };
                });
                res.json(respuesta);
            } catch (err) {
...
app.post('/api/reentrenar-ia', async (req, res) => {
    ...
    exec(`python3 ${scriptPath}`, (error, stdout, stderr) => {
        if (error) return res.status(500).json({ error: 'Error al reentrenar el modelo' });
        console.log(stdout);
